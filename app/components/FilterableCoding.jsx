...
// Technology mapping - maps portfolio tech to filter categories
const techMapping = {
  'Unity': ['Unity', 'Game Dev'],
  'Cloud': ['Cloud', 'Google Cloud', 'AWS', 'Azure'],
  'C#': ['C#'],
  'Animation': ['Animation'],
  'Mobile': ['Mobile', 'iOS'],
...
// All available filter technologies
const filterTechs = [
  'All',
  'AI',
  'Animation',
...
// Portfolio items with their filterable tech tags
const portfolioWithTech = Object.entries(portfolioItems).map(([slug, item]) => ({
  slug,
  ...item,
  filterableTech: item.tech.flatMap(tech => techMapping[tech] || [tech])
}))

// Company logo data with associated portfolio slugs
const companyLogos = [
  { slug: 'accuweather', alt: 'AccuWeather', src: '/logos/AccuWeather.svg', className: 'h-9 grayscale brightness-200 contrast-50 hover:scale-150 transition' },
  { slug: 'ally', alt: 'Ally', src: '/logos/ally.svg', className: 'h-9 grayscale invert brightness-200 contrast-50 hover:scale-150 transition' },
  { slug: 'aep', alt: 'American Electric Power', src: '/logos/aep.svg', className: 'h-12 grayscale invert brightness-125 hover:scale-150 transition' },
  { slug: 'amway', alt: 'Amway', src: '/logos/amway.png', className: 'h-12 grayscale invert brightness-200 contrast-75 hover:scale-150 transition' },
  { slug: 'barefoot', alt: 'Barefoot Wine', src: '/logos/barefoot.svg', className: 'h-9 grayscale invert brightness-200 contrast-50 hover:scale-150 transition' },
  { slug: 'fiat', alt: 'Fiat', src: '/logos/Fiat.svg', className: 'h-12 grayscale invert brightness-150 hover:scale-150 transition' },
  { slug: 'magna', alt: 'Magna', src: '/logos/magna.svg', className: 'h-9 grayscale brightness-200 contrast-0 hover:scale-150 transition' },
  { slug: 'marvel', alt: 'Marvel', src: '/logos/marvel.svg', className: 'h-9 brightness-75 contrast-50 hover:scale-150 transition' }
]

// Non-clickable company logos
const staticLogos = [
  { alt: 'Boca Code', src: '/logos/boca-code.svg', className: 'h-9 brightness-75' },
  { alt: 'Bowling Green State University', src: '/logos/bgsu-rev.svg', className: 'h-7 grayscale brightness-200 contrast-50' },
  { alt: 'CIRES', src: '/logos/cires.png', className: 'h-9 brightness-150 contrast-50' },
  { alt: 'Cornerstone Learning Community', src: '/logos/clc.png', className: 'h-12 grayscale brightness-200 contrast-50' },
  { alt: 'Curator', src: '/logos/curator.png', className: 'h-9 grayscale' },
  { alt: 'Eight O`Clock Coffee', src: '/logos/eight_o_clock.svg', className: 'h-12 grayscale brightness-75' },
  { alt: 'Frenchie Ballers', src: '/logos/frenchie-ballers.png', className: 'h-12 brightness-75' },
  { alt: 'George P Johnson', src: '/logos/GPJ.png', className: 'h-12 brightness-125' },
  { alt: 'Hemp Temps', src: '/logos/hemp-temps.png', className: 'h-12 brightness-75' },
  { alt: 'Imanyco', src: '/logos/imanyco.png', className: 'h-9 brightness-75' },
  { alt: 'JetRide', src: '/logos/jetride.png', className: 'h-6 brightness-75' },
  { alt: 'Koda', src: '/logos/koda.png', className: 'h-12 brightness-75' },
  { alt: 'Mack Industrial', src: '/logos/mack-industrial.png', className: 'h-12 invert opacity-60' },
  { alt: 'Magic City Cup', src: '/logos/magic-city-cup.png', className: 'h-12 brightness-75 contrast-75' },
  { alt: 'MV Realty', src: '/logos/mvrealty.png', className: 'h-9 grayscale contrast-75' },
  { alt: 'MyPhoto', src: '/logos/myphoto.png', className: 'h-6 brightness-75 contrast-75' },
  { alt: 'NASA', src: '/logos/nasa.png', className: 'h-14 brightness-75' },
  { alt: 'Nebular Agency', src: '/logos/nebular2.png', className: 'h-9 brightness-75' },
  { alt: 'Nintendo Switch', src: '/logos/switch.svg', className: 'h-14 invert contrast-75' },
  { alt: 'PBS Kids', src: '/logos/pbs-kids.svg', className: 'h-12 grayscale brightness-75 contrast-75' },
  { alt: 'Shelby USA', src: '/logos/shelby.png', className: 'h-14 invert brightness-75' },
  { alt: 'Site Impact', src: '/logos/site-impact.png', className: 'h-9 grayscale contrast-50 brightness-125' },
  { alt: 'Sprint', src: '/logos/Sprint_Corporation_Logo.svg', className: 'h-12 brightness-125' },
  { alt: 'Sun Country Airlines', src: '/logos/sun-country.webp', className: 'h-12 grayscale brightness-125 contrast-50' }
]

export default function FilterableCoding() {
  const [activeFilter, setActiveFilter] = useState('All')

  // Filter portfolio items based on active filter
  const filteredPortfolio = useMemo(() => {
    if (activeFilter === 'All') return portfolioWithTech
    return portfolioWithTech.filter(item =>
      item.filterableTech.includes(activeFilter)
    )
  }, [activeFilter])

  // Filter company logos based on filtered portfolio
  const filteredLogos = useMemo(() => {
    if (activeFilter === 'All') return companyLogos
    const filteredSlugs = new Set(filteredPortfolio.map(item => item.slug))
    return companyLogos.filter(logo => filteredSlugs.has(logo.slug))
  }, [filteredPortfolio, activeFilter])
  ...
  const techCounts = useMemo(() => {
    ...
    filterTechs.forEach(tech => {
      if (tech === 'All') {
        counts[tech] = portfolioWithTech.length
      } else {
        counts[tech] = portfolioWithTech.filter(item =>
          item.filterableTech.includes(tech)
        ).length
      }
    })
    return counts
  ...
  return (
    <section data-aos='fade-up' data-aos-duration='500' data-aos-offset='100'>
      <div className='py-8 lg:py-16 mx-auto my-auto max-w-screen-xl px-4'>
        ...
        <div className='flex flex-wrap justify-center my-6 lg:mb-8 w-full mx-auto text-center'>
          {filterTechs.map(tech => (
            <button
              key={tech}
              onClick={() => setActiveFilter(tech)}
              className={`text-sm font-medium me-2 mb-2 px-2.5 py-0.5 rounded transition-all duration-200 ${
                activeFilter === tech
                  ? 'bg-orange-600 text-orange-100 shadow-lg scale-105'
                  : techCounts[tech] > 0
                    ? 'bg-blue-900 text-blue-300 hover:bg-blue-800 hover:scale-105 cursor-pointer'
                    : 'bg-gray-800 text-gray-500 cursor-not-allowed opacity-50'
              }`}
              disabled={techCounts[tech] === 0}
            >
              {tech} {techCounts[tech] > 0 && `(${techCounts[tech]})`}
            </button>
          ))}
        </div>
        ...
        {activeFilter !== 'All' && (
          <div className='text-center mb-6'>
            <p className='text-stone-400 text-sm'>
              Showing {filteredLogos.length} project{filteredLogos.length !== 1 ? 's' : ''} using <span className='text-orange-400 font-medium'>{activeFilter}</span>
              <button
        ...
        {/* Filtered Company Logos Grid */}
        <div className='grid grid-cols-2 gap-8 text-stone-400 sm:gap-12 md:grid-cols-3 lg:grid-cols-6 transition-all duration-300'>
          {/* Clickable Portfolio Logos */}
          ...
          {/* Static Company Logos (always visible when showing all) */}
          {activeFilter === 'All' && staticLogos.map((logo, index) => (
        ...
        {/* No Results Message */}
        {filteredLogos.length === 0 && activeFilter !== 'All' && (
          <div className='text-center py-12'>
            <p className='text-stone-400 text-lg mb-4'>
              No projects found using <span className='text-orange-400 font-medium'>{activeFilter}</span>
            </p>
            <button
              onClick={() => setActiveFilter('All')}
